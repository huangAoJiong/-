...
public class Room {
    ...
    private final static long FIRECRACKER_DURATION;
    ...
    @Autowired
    public Room(ApplicationProperties applicationProperties, ObjectMapper objectMapper) {
    ...
    private synchronized void strandsNext() {
        if (strandsStatus.isIgnited() && !strandsStatus.isFinished()) {
            ...
            if (nextIndex < playerList.size()) {
                ...
                sendGameDataToPlayerAsync(player);
                if (applicationProperties.isBlessingEnabled() && applicationProperties.getBlessings() != null && !applicationProperties.getBlessings().isEmpty()) {
                    if (timerTask != null) {
                        timerTask.cancel();
                    }
                    timerTask = new TimerTask() {
                    ...
                    timer.schedule(timerTask, FIRECRACKER_DURATION);
                }
            }
        }
    }

    private synchronized void showBlessings() {
        Random random = new Random();
        List<String> blessings = applicationProperties.getBlessings();
        for (Player player : playerList) {
            player.getBlessing().show(blessings.get(Math.abs(random.nextInt() % blessings.size())));
        }
        sendGameDataToPlayersAsync();
    }
    ...
    private synchronized PlayerGameData createPlayerGameData(Player player) {
        ...
        Blessing blessing = player.getBlessing();
        BlessingDto blessingDto = new BlessingDto();
        blessingDto.setVisible(blessing.isVisible());
        blessingDto.setContent(blessing.getContent());
        playerGameData.setBlessing(blessingDto);
