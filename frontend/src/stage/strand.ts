...
export default class Strand {
    ...
    constructor(type: StrandType) {
        ...
        for (let i = 0; i < firecrackersCount; i++) {
            const firecracker = new Firecracker(((this.type === 'single' || this.type === 'last') && i < BIG_FIRECRACKER_COUNT) ? 'big' : 'normal')
    ...
    reset(type?: StrandType) {
        ...
        for (let i = 0; i < firecrackersCount; i++) {
            ...
            if (i < this.firecrackers.length) {
                ...
                firecracker.reset(((this.type === 'single' || this.type === 'last') && i < BIG_FIRECRACKER_COUNT) ? 'big' : 'normal')
            } else {
                firecracker = new Firecracker(((this.type === 'single' || this.type === 'last') && i < BIG_FIRECRACKER_COUNT) ? 'big' : 'normal')
                this.fireCrackersContainer.add(firecracker.getContainer())
            }
            firecracker.getContainer().attr({
                y: (i + firecrackersOffset) * UNIT_LENGTH,
